    ...
    if message.chat.id == SUPPORT_GROUP :
                ...
                if total != 0:
                    ...
                    msg = await message.reply_text(script.SUPPORT_GRP_MOVIE_TEXT.format(message.from_user.mention() , total) ,             reply_markup=InlineKeyboardMarkup([
                        [ InlineKeyboardButton('𝐆𝐄𝐓 𝐅𝐈𝐋𝐄𝐒 𝐅𝐑𝐎𝐌 𝐇𝐄𝐑𝐄 😉' , url=link)]
                        ]))
...
async def next_page(bot, query):
    ...
    btn.insert(0,[
        InlineKeyboardButton("𝐒𝐄𝐍𝐃 𝐀𝐋𝐋", callback_data=batch_link),
        InlineKeyboardButton("𝐋𝐀𝐍𝐆𝐔𝐀𝐆𝐄", callback_data=f"languages#{key}#{offset}#{req}"),
        ])
    btn.insert(1, [
        InlineKeyboardButton("𝐐𝐔𝐀𝐋𝐈𝐓𝐘", callback_data=f"qualities#{key}#{offset}#{req}"),
        InlineKeyboardButton("𝐘𝐄𝐀𝐑", callback_data=f"years#{key}#{offset}#{req}"),
        InlineKeyboardButton("𝐒𝐄𝐀𝐒𝐎𝐍", callback_data=f"seasons#{key}#{offset}#{req}")
    ])
...
async def season_search(client: Client, query: CallbackQuery):
    ...
    if n_offset== '':
        btn.append(
            [InlineKeyboardButton(text="🚸 𝐍𝐎 𝐌𝐎𝐑𝐄 𝐏𝐀𝐆𝐄𝐒 🚸", callback_data="buttons")]
        )
    elif n_offset == 0:
        btn.append(
    ...
    btn.append([
        InlineKeyboardButton(text="⪻ 𝐁𝐀𝐂𝐊 𝐓𝐎 𝐌𝐀𝐈𝐍 𝐏𝐀𝐆𝐄", callback_data=f"next_{req}_{key}_{orginal_offset}"),])
    await query.message.edit_text(cap + links + del_msg, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        #add back button
        buttons = [[
            ('⇆ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs ⇆', url=f'http://t.me/{temp.U_NAME}?startgroup=start')],
            [('⪻ 𝐁𝐀𝐂𝐊', callback_data='features')]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
